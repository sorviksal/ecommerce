...
window.addEventListener('scroll', function() {
  ...
  const productList = document.getElementById("productList");
        ...
        function showProductModal(product) {
          ...
          modalQty.textContent = product.qty;
        ...
        function renderProduct(product) {
          productList.innerHTML += `
            <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6">
              <div class="card product-card h-100" onclick='showProductModal(${JSON.stringify(product)})'>
                <img src="${product.image}" class="product-img">
                <div class="card-body d-flex flex-column">
                  <h5 class="card-title">${product.name}</h5>
                  <p class="text-primary">(${product.qty})</p>
                  <p class="price mt-auto">៛${product.price.toLocaleString()}</p>
                  <button class="btn btn-primary w-100">មើលលម្អិត</button>
                </div>
              </div>
            </div>
          `;
